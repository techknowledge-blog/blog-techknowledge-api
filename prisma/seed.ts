...
async function main() {
  ...
  const TECHKNOWLEDGE_PONTO_ZERO = await prisma.post.create({
    data: {
      ...
      previewContent:
        '<p>Como surgiu a ideia de criar esse blog? Qual a finalidade do nosso projeto? Nesse primeiro post, vamos nos apresentar e  falar um pouco da motivação por trás do TechKnowledge!</p>',
  ...
  const QUEM_SOMOS_NOS = await prisma.post.create({
    data: {
      ...
      previewContent:
        '<p>Quem é a Leticia e o Mikael? São dois aventureiros? São dois alíenigenas de outro planeta? Ou são só dois devs que gostam muito de tecnologia? Vem descubrir um pouco sobre a jornada desses dois jovens devs.</p>',
      slug: 'quem-somos-nos',
      isPublished: true,
    },
  });
